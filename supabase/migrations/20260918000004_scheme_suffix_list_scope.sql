-- Real TKM schemes apply to a curated list of suffixes within a model (e.g. 9 specific Glanza
-- variants out of the full lineup), not "one suffix" or "every suffix" — the single nullable
-- suffix_scope text column can't express that without duplicating the whole scheme + its line
-- items once per suffix. Replace it with an array (null = every suffix of the model, same as
-- before).

alter table schemes add column suffix_scope_list text[];
alter table schemes drop column suffix_scope;
  ...
  if exists (
    ...
    where not exists (
      ...
      where sli.id = sid
        ...
        and (s.suffix_scope_list is null or v_suffix = any(s.suffix_scope_list))
