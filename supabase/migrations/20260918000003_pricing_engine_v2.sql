-- Two fixes surfaced by importing real TKM scheme/price data:
--
-- 1. Insurance: prefer an exact insurance_pricing match (variant/model + state + date) over the
--    generic idv_percentage/flat formula, since real premiums are flat per-variant rack rates,
--    not a computed percentage.
--
-- 2. Dealer margin on non-customer-facing scheme lines: real "Dealer / Business Head Scheme" and
--    "Bulk Deal Scheme" rows are pure TKM-to-dealer incentive money — e.g. a row with
--    tkm_share=80000, dealer_share=NULL is TKM paying the dealer 80,000/car, not a cost-split of
--    a customer discount. The old logic only added dealer_share to margin, so these showed as
--    zero margin impact. Since these items are never customer-selectable, the dealer receives
--    the full amount regardless of which party notionally "funds" it — margin should include
--    tkm_share + dealer_share together, not dealer_share alone. Customer-facing items are
--    unchanged: the dealer's margin there is reduced only by their own dealer_share portion of a
--    discount actually extended to the customer.
...
begin
  ...
  loop
    if rec.is_customer_facing then
      ...
      -- Pure TKM-to-dealer incentive money: the dealer receives the full amount regardless of
      -- which column nominally carries it, and it never reduces the customer's price.
      v_dealer_margin := v_dealer_margin + rec.tkm_share + rec.dealer_share;
    end if;
  end loop;
  ...
  for rec in
    select sli.tkm_share, sli.dealer_share
    from scheme_line_items sli
    join schemes s on s.id = sli.scheme_id
  ...
  if v_insurance_plan_id is not null then
    ...
    -- Prefer an exact variant/model-scoped rate over the plan's generic formula.
    select amount into v_variant_premium
    from insurance_pricing
    where insurance_plan_id = v_insurance_plan_id and state = v_state
      and (variant_id = v_variant_id or (variant_id is null and model_id = v_model_id))
      and effective_from <= v_as_of_date and (effective_to is null or effective_to >= v_as_of_date)
    order by variant_id nulls last, effective_from desc
    limit 1;

    if v_variant_premium is not null then
      v_base_premium := v_variant_premium;
    elsif v_basis = 'idv_percentage' then
